...
void ScaleToUnitBoxProcess::Execute( aiScene* pScene)
{
  ...
  for( unsigned int a = 0; a < pScene->mNumMeshes; a++) {
    if(this->ScaleMesh( pScene->mMeshes[a])) {
      bHas = true;
    }
  }
  if (bHas) {
  ...
  else DefaultLogger::get()->debug("ScaleToUnitBoxProcess finished. "
    "Mesh vertices have already been scaled to unit box");
}

// ------------------------------------------------------------------------------------------------
// Scales all vertices of the given mesh to conform unit box.
bool ScaleToUnitBoxProcess::ScaleMesh( const aiMesh* pMesh) const
{
